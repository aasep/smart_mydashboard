...
$jml_tgl=countTglDashZIMBRA();

  $q_loop= " select distinct entry_date FROM dashboard_activity where sys_name='ZIMBRA' ";
  $RQ   = $db->Execute($q_loop);
  while(!$RQ->EOF){
        $z=1;
        $date_dash= date('Y-m-d',strtotime($RQ->fields['entry_date']));
        $jml_par= countTglDashZIMBRA2($date_dash);
                echo '{';
                echo '"date":"'.$date_dash.'",';

        ####### Query expand  ########
        $query1 = " select entry_date,control,value as jml from dashboard_activity where entry_date = '".$date_dash."' and sys_name='ZIMBRA' ";
        //echo $query1."<br>";
...
<?php
 $control_label=array();
 $jml_array=0;
 $q_array= " select distinct control FROM dashboard_activity where sys_name='ZIMBRA' ";
